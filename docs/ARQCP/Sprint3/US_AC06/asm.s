...
extract_token:
    pushq %rdi                     # Guarda o valor original de %rdi na stack
    ...
    je return_if_invalid           # jump para 'return_if_invalid' se %ecx é 0
    movq %rsi, %r8                 # Guarda o valor original de %rsi em %r8
    xor %rcx, %rcx                 # define o %rcx a 0
    xor %rax, %rax                 # define o %rax a 0
...
return_parameters:
    movq %r8, %rsi                 # Restaura %rsi ao valor original
    popq %rdi                      # Restaura %rdi do topo da stack
